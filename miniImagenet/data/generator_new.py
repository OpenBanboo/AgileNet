...
class Generator(data.Dataset):
    def __init__(self, root, args, partition='train', dataset='omniglot',transform=None, shot=1, way=5 , fewshot = False):
        self.root = root
        ...
        if dataset == 'omniglot':
            ...
            if self.partition == "both":
                self.data_train, self.label_encoder_train = self.loader.load_dataset('train', self.size)
                self.data_test, self.label_encoder_test = self.loader.load_dataset('test', self.size)
            else:
                self.data, self.label_encoder = self.loader.load_dataset(self.partition, self.size)
            #print(np.asarray(self.data.values()).shape)
            data_array = np.asarray(self.data_test.values())
            ...
            #few_classes = [12,13,14,16,17]
            random.seed(3) # for 5 shot 66.77%
            rnd = random.sample(range(600),self.shot)
            #if self.fewshot:
            #    print("Reandom seeds:", rnd)
            ...
            #print("Labels: ", self.few_label)

            #print(np.asarray(self.data.values()).shape)
            data_array = np.asarray(self.data_train.values())
            #print(np.shape(data_array))
            self.data_arr = data_array.reshape(-1,*data_array.shape[2:])
            #print(np.shape(self.data_arr))
            few_classes = range(64)
            #few_classes = [12,13,14,16,17]
            random.seed(3) # for 5 shot 66.77%
            rnd = random.sample(range(600),self.shot)
            #if self.fewshot:
    ...
    def __getitem__(self, index):
        ...
        if self.fewshot == True:
            ...
            img, target = self.data_arr[index], index/600
    ...
    def __len__(self):
        if self.fewshot == True and self.partition == 'both':
            return 64+self.way*self.shot
        elif self.fewshot == True:
